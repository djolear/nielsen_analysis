...
# Set data path
data_path <- "D:/data/nielsen/calories_extracts/qfahpd_health_calories_imputed_sc_by_household_quarterly/"
...
for(i in 1:length(file_list$file_list)) {
  assign(
    paste0("qhc_isc_qr_", str_extract(file_list$file_list[i], "[[:digit:]]+")), 
    add_secondary_and_save_two_person(
      ...
      file_name_stem = "qhc_isc_qr_sec_tp_"
    )
  )
