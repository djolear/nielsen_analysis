...
# Set data path
data_path <- "D:/data/nielsen/spend_extracts/qfahpd_health_spend_by_household_monthly/"
...
for(i in 1:length(file_list$file_list)) {
  assign(
    paste0("qh_spend_by_household_monthly_", str_extract(file_list$file_list[i], "[[:digit:]]+")), 
    add_secondary_and_save(
      ...
      file_name_stem = "qh_spend_by_household_monthly_wide_secondary_"
    )
  )
