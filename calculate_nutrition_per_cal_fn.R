...
saturated_fat_per_cal_fn <- function(df){
  niel_df_saturated_fat <-
    df  %>% 
    filter(!is.na(saturated_fat) & !is.na(calories))
...
add_sugars_per_cal_fn <- function(df){
  niel_df_add_sugars <-
    df  %>% 
    filter(!is.na(add_sugars) & !is.na(calories))
  
  total_add_sugars <-
    niel_df_add_sugars %>% 
    group_by(household_code) %>% 
    summarise(
      total_add_sugars = sum(add_sugars, na.rm = TRUE)
    )
...
nutrition_per_cal_fn <- function(year){
  ...
  total_sugar <- sugar_per_cal_fn(df)
  total_saturated_fat <- saturated_fat_per_cal_fn(df)
  total_sodium <- sodium_per_cal_fn(df)
  total_add_sugars <- add_sugars_per_cal_fn(df)
  total_produce <- produce_per_cal_fn(df)
  ...
  nutr_per_cal <-
    nutr_per_cal %>% 
    mutate(
      hei_mod = produce_cal - add_sugar_cal - sf_cal - sod_cal
    )
