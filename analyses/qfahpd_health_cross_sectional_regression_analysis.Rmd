...
lm_dri_cal <-
  lm(
    scale(yes_scale) ~
      scale(income_demo_ranger_sar_scale) +
      income_scale +
...
lm_dri_spn <-
  lm(
    ...
    data = 
      qh_spend_by_household_monthly %>% 
      filter(year %in% c(2004:2016)) %>% 
      filter(Male_Head_Education != 0 & Female_Head_Education != 0)
  )

summary(lm_dri_spn)
regclass::VIF(lm_dri_spn)
lm.beta::lm.beta(lm_dri_spn)
...
summary(lm_gri_cal)
regclass::VIF(lm_gri_cal)
lm.beta::lm.beta(lm_gri_cal)
...
summary(lm_gri_spn)
regclass::VIF(lm_gri_spn)
lm.beta::lm.beta(lm_gri_spn)
