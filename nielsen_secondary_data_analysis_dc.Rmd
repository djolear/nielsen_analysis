---
title: "Nielsen Secondary Data Analysis - Daniel Categories"
...
spend_per_dc_17 <- 
  calculate_spend_per_dc_fn(year)

spend_per_dc_17 <-
  bind_zip_code_census_data_function(spend_per_dc_17)  
```
...
effect("inc_mid", lm_inc) %>% 
  ...
  geom_vline(xintercept = mean(spend_per_dc_17$inc_mid, na.rm = TRUE)) +
  geom_vline(xintercept = mean(spend_per_dc_17$inc_mid, na.rm = TRUE) + sd(spend_per_dc_17$inc_mid, na.rm = TRUE)) +
  geom_vline(xintercept = mean(spend_per_dc_17$inc_mid, na.rm = TRUE) - sd(spend_per_dc_17$inc_mid, na.rm = TRUE)) +
  labs(
    x = "own income",
...
effect("inc_diff", lm_inc_diff) %>% 
  ...
  geom_ribbon(aes(ymin = fit - se, ymax = fit + se), alpha  = 0.2) +
  scale_x_continuous(labels = scales::comma) +
  geom_vline(xintercept = mean(spend_per_dc_17$inc_diff, na.rm = TRUE)) +
  geom_vline(xintercept = mean(spend_per_dc_17$inc_diff, na.rm = TRUE) + sd(spend_per_dc_17$inc_diff, na.rm = TRUE)) +
  geom_vline(xintercept = mean(spend_per_dc_17$inc_diff, na.rm = TRUE) - sd(spend_per_dc_17$inc_diff, na.rm = TRUE)) +
  labs(
