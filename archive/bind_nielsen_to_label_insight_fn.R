...
bind_labels_to_purchases_fn <- function(year) {
  ...
  li_upc_update <- read_csv(paste0(machine_path, "research/projects/niel/nielsen_analysis/li_upc_update_011021.csv"))
    ...
    left_join(
      li_upc_update %>%
        dplyr::select(
          ...
          calories,
          saturated_fat,
          # cal_sf = `Calories From Saturated Fat %`,
          # cal_sug = `Calories From Sugar %`,
          sodium = Sodium,
          # fiber = `Dietary Fiber`,
          # protein = Protein
        ),
