...
nielsen_read_add2_write <- function(df_path, current_year) {
  ...
  write_csv(df, paste0(machine_path, data_path, "group_spend_by_household_food_only_wide_secondary_", current_year, ".csv"))
}
...
for(i in 1:length(file_list$file_list)) {
  assign(
    paste0("nielsen_group_by_household_food_only_", str_extract(file_list$file_list[i], "[[:digit:]]+")), 
