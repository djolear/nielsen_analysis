...
nielsen_read_add_secondary_write <- function(df_path, current_year) {
  ...
  write_csv(df, paste0(data_path, "tfp_spend_by_household_quarterly_wide_secondary_", current_year, ".csv"))
  
  return(df)
}

data_path <- "D:/data/nielsen/tfp_spend_by_household_quarterly/"
...
for(i in 1:length(file_list$file_list)) {
  assign(
    paste0("tfp_by_household_quarterly_", str_extract(file_list$file_list[i], "[[:digit:]]+")), 
